...
fileVidListAviWext= [file for file in fileList if file.endswith(".mov")]
fileVidNameList = []
...
dirSaveImgPath = os.path.join(basicPath, fileVidNameListAviSorted[0])

# browsing video files for saving each frame (png file)
for i in range(len(fileVidListAviWext)):
  dirSaveImgPath = os.path.join(basicPath, fileVidNameListAviSorted[i])
  vidFilePath = os.path.join(basicPath, fileVidNameListAviSorted[i] + ".mov")
  ...
  if os.path.isfile(vidFilePath):
    ...
    for j in range(length):
      ...
      ret, frame = cap.read()
      if ret:
        imgSaveFileName = str(dirSaveImgPath) + "/" + str(j + 1) + ".png"
