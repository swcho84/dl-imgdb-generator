...
void CvtKtt2Bbox::MainLoopBboxGenerator() {
  // 2nd, reading, matching, resizing raw bbox data and saving resized bbox data
  // w.r.t the xml type
  ...
  for (size_t i = 0; i < vecImgFileNm.size(); i++) {
    ...
    strStreamFileName << std::setfill('0')
                      << std::setw(cfgParam_.nKttXmlFileNmDigit) << i;
    ...
    for (auto kk = 0; kk < vecKittyDB[i].size(); kk++) {
      for (auto kkk = 0; kkk < cfgParam_.vecAnnoKttDB.size(); kkk++) {
        ...
        if (cfgParam_.vecAnnoKttDB[kkk].strLabel ==
            vecKittyDB[i][kk].strLabel) {
          ...
          // // for debugging
          // ROS_INFO("[%d]label(%s), bbox(tl,br):(%d,%d)////(%d,%d)",
          // (int)(kk), vecKittyDB[i][kk].strLabel.c_str(),
          //         (int)(vecKittyDB[i][kk].fBbox[0]),
          //         (int)(vecKittyDB[i][kk].fBbox[1]),
...
void CvtKtt2Bbox::MainLoopImgResizer() {
  ...
  for (size_t i = 0; i < vecImgFileNm.size(); i++) {
    ...
    Mat imgResize;
    resize(imgRaw, imgResize,
           Size(cfgParam_.nKttWidthRef, cfgParam_.nKttHeightRef), 0, 0,
           INTER_NEAREST);
    ...
    strStreamImgFileName << std::setfill('0')
                         << std::setw(cfgParam_.nKttImgFileNmDigit) << i;
    strStreamImgFileName << "." + cfgParam_.strKttImgExt;

    // making the full file path
    string strCvtImgFile;
    strCvtImgFile =
        cfgParam_.strKttCvtImgFolderPath + strStreamImgFileName.str();
...
bool CvtKtt2Bbox::GetSizeCalcFlag() { return bSizeCalcFlag; }
